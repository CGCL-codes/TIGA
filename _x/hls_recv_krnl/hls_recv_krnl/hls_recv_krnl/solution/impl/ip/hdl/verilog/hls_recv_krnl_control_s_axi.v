...
// ==============================================================
`timescale 1ns/1ps
module hls_recv_krnl_control_s_axi
#(parameter
    ...
    output wire [63:0]                   expectedRxByteCnt,
    output wire [63:0]                   out_time,
...
localparam
    ...
    ADDR_IER                      = 6'h08,
    ADDR_ISR                      = 6'h0c,
    ADDR_USECONN_DATA_0           = 6'h10,
    ADDR_USECONN_CTRL             = 6'h14,
    ADDR_BASEPORT_DATA_0          = 6'h18,
    ADDR_BASEPORT_CTRL            = 6'h1c,
    ADDR_DELAY_DATA_0             = 6'h20,
    ADDR_DELAY_CTRL               = 6'h24,
    ADDR_EXPECTEDRXBYTECNT_DATA_0 = 6'h28,
    ADDR_EXPECTEDRXBYTECNT_DATA_1 = 6'h2c,
    ADDR_EXPECTEDRXBYTECNT_CTRL   = 6'h30,
    ADDR_OUT_TIME_DATA_0          = 6'h34,
    ADDR_OUT_TIME_DATA_1          = 6'h38,
    ADDR_OUT_TIME_CTRL            = 6'h3c,
    WRIDLE                        = 2'd0,
    WRDATA                        = 2'd1,
    WRRESP                        = 2'd2,
    WRRESET                       = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_expectedRxByteCnt = 'b0;
    reg  [63:0]                   int_out_time = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_EXPECTEDRXBYTECNT_DATA_0: begin
                    rdata <= int_expectedRxByteCnt[31:0];
                end
                ADDR_EXPECTEDRXBYTECNT_DATA_1: begin
                    rdata <= int_expectedRxByteCnt[63:32];
                end
                ADDR_OUT_TIME_DATA_0: begin
                    rdata <= int_out_time[31:0];
                end
                ADDR_OUT_TIME_DATA_1: begin
                    rdata <= int_out_time[63:32];
                end
            endcase
        end
    end
...
assign expectedRxByteCnt = int_expectedRxByteCnt;
assign out_time          = int_out_time;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
// int_expectedRxByteCnt[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_expectedRxByteCnt[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_EXPECTEDRXBYTECNT_DATA_0)
            int_expectedRxByteCnt[31:0] <= (WDATA[31:0] & wmask) | (int_expectedRxByteCnt[31:0] & ~wmask);
    end
end

// int_expectedRxByteCnt[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_expectedRxByteCnt[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_EXPECTEDRXBYTECNT_DATA_1)
            int_expectedRxByteCnt[63:32] <= (WDATA[31:0] & wmask) | (int_expectedRxByteCnt[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OUT_TIME_DATA_0)
            int_out_time[31:0] <= (WDATA[31:0] & wmask) | (int_out_time[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_out_time[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUT_TIME_DATA_1)
            int_out_time[63:32] <= (WDATA[31:0] & wmask) | (int_out_time[63:32] & ~wmask);
    end
end
